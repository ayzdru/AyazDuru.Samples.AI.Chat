...
var micAudioSource = new MicrophoneInputSource(deviceNumber: 0);
...
async Task ShowTranscriptAsync()
{
    ...
    await foreach (var transcription in realTimeTranscriptor.TranscribeAsync(micAudioSource))
    {
        ...
        {
            textToWrite = $"-{segmentRecognized.Segment.StartTime}-{segmentRecognized.Segment.StartTime + segmentRecognized.Segment.Duration}:{segmentRecognized.Segment.Text}";
        }
        //else if (transcription is RealtimeSegmentRecognizing segmentRecognizing)
        //{
        //    textToWrite = $"*{segmentRecognizing.Segment.StartTime}-{segmentRecognizing.Segment.StartTime + segmentRecognizing.Segment.Duration}:{segmentRecognizing.Segment.Text}";
        //}
        //else if (transcription is RealtimeSessionStarted sessionStarted)
        //{
        //    textToWrite = $"SessionId: {sessionStarted.SessionId}";
        //}
        //else if (transcription is RealtimeSessionStopped sessionStopped)
        //{
        //    textToWrite = $"SessionId: {sessionStopped.SessionId}";
        //}
        //else
        //{
...
ISpeechTranscriptorFactory GetWhisperTranscriptor()
{
    ...
    var ggmlModelPath = "ggml-base.bin";
...
IVadDetectorFactory GetSileroVadDetector()
{
    ...
    // Or execute `downloadModels.ps1` script in the root of this repository
    var sileroOnnxPath = "C:\\Projects\\ayzdru\\AyazDuru.Samples.AI.Chat\\source\\AyazDuru.Samples.AI.Chat.Console\\bin\\Debug\\net9.0\\silero_vad.onnx";
